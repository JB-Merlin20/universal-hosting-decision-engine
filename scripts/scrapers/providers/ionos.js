...
const URL_SHARED = "https://www.ionos.com/hosting/web-hosting";

const PLANS = [
  { id: "ionos-essential", keywords: ["essential"] },
  { id: "ionos-plus", keywords: ["plus"] },
  { id: "ionos-ultimate", keywords: ["ultimate"] },
];
...
async function scrape(currentProviders) {
  ...
  try {
    ...
    const cardSelectors = [
      ...
      "[class*='product-card']", "[class*='offer']",
    ];
    ...
    for (const sel of cardSelectors) {
      $(sel).each((_, el) => {
        ...
        const renewalText = block.find("[class*='renewal'], [class*='regular'], [class*='after']").first().text();
    ...
    console.warn(`  IONOS scrape failed: ${err.message}`);
  }
